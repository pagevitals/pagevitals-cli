...
async function runTests(options, apiToken, details) {
  ...
  let data = await makeApiCall(endpoint, 'post', options, apiToken, options.output === 'console');
...
async function pollForCompletion(initialData, options, apiToken, pollingDetails) {
  const spinner = options.output === 'console' ? ora('Waiting for test completion...').start() : null;
